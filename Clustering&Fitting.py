...
def load_and_filter_data(file_path, agricultural_land_indicator, population_indicator, year):
    """
    ...
    data1 = df[df['Indicator Name'] == agricultural_land_indicator][['Country Name', year]].rename(
        columns={year: agricultural_land_indicator})
    data2 = df[df['Indicator Name'] == population_indicator][['Country Name', year]].rename(
        columns={year: population_indicator})
    merged_data = pd.merge(data1, data2, on='Country Name', how='outer').dropna().reset_index(drop=True)
    return merged_data
...
def apply_kmeans_and_plot_subplots(df1, df2, cluster_columns, num_clusters=4, title1='', title2=''):
    """
    ...
    for data_frame, subplot_title, axis in zip([df1, df2], [title1, title2], axes):
        ...
        scatter_plot = sns.scatterplot(
            x=cluster_columns[0], y=cluster_columns[1], hue='cluster', data=data_frame, ax=axis)
        axis.scatter(cluster_centers[:, 0], cluster_centers[:, 1], marker='+', s=50, c='black', label='Cluster Centers')
        axis.set_title(subplot_title)
        axis.set_ylabel('Total Population (Millions)')  # Updated y-axis label
        axis.set_xlabel('Agricultural Land (% of Land Area)')  # Updated x-axis label
        legend_handles = [Line2D([0], [0], marker='o', color='w', markerfacecolor=f'C{i}', markersize=8) for i in
                          range(num_clusters)]
        legend_handles.append(Line2D([0], [0], marker='+', color='black', markersize=8))
        legend_labels = [f'Cluster {i + 1}' for i in range(num_clusters)] + ['Cluster Centers']
        axis.legend(handles=legend_handles, labels=legend_labels, loc='upper left', fontsize='small', handlelength=0.5,
                    handletextpad=0.5)
    plt.tight_layout()


def plot_polynomial_regression_with_error(x, y, degree, ax, title, curve_color, data_color):
    """
    ...
    ax.plot(x, y, marker='.', linestyle='-', label='Actual Data', color=data_color)
    ...
    ax.fill_between(all_years_extended, lower_bound, upper_bound, color=curve_color, alpha=0.3,
...
data_selected = df[(df['Country Name'].isin(selected_countries)) & (df['Indicator Name'] == indicator_name)].reset_index(
    drop=True)
...
data_forecast = data_forecast[(data_forecast['Year'] >= 1980) & (data_forecast['Year'] <= 2020)]

predictions = {}
all_years_extended = list(range(1980, 2026))

actual_data_colors = ['red', 'blue']
...
for country, data_color in zip(selected_countries, actual_data_colors):
    ...
    plot_polynomial_regression_with_error(x_country, y_country, degree=3, ax=ax,
                                          title=f'Agricultural Land Area Forecast for {country}',
                                          curve_color=curve_color, data_color=data_color)
